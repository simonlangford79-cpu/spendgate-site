...
const INIT_REQS = [
  ...
  { id:"JG-0014", venue:"Site 3 — North",   sup:"Screwfix",       cat:"Maintenance",  amt:420,  by:"Chris Adams",  desc:"Drainage repair materials",                    why:"Flooding near access road — playability affected",    pri:true,  status:"Approved", created:"2 days", pref:true,  last:388, note:"Order ASAP — site access affected" },
...
export default function Demo() {
  ...
  const EXCEPTIONS = reqs.map(r => {
    const flags: { type: string; title: string; body: string; bg: string; bd: string; c: string }[] = [];
    const vb = VB[r.venue] || { t: 5000, u: 2500 };
    if (r.pri && r.status === "Pending") flags.push({ type: "urgent", title: "Urgent pending", body: "Marked as operationally urgent — needs rapid response.", bg: "#eff6ff", bd: "#bfdbfe", c: "#1e40af" });
    if (!r.pref) flags.push({ type: "supplier", title: "Off-preferred supplier", body: r.sup + " is not on the preferred supplier list for " + r.cat + ".", bg: "#fff7ed", bd: "#fed7aa", c: "#92400e" });
    if ((vb.u + r.amt) > vb.t) flags.push({ type: "budget", title: "Budget exception", body: "This request would exceed the monthly budget for " + r.venue + ".", bg: "#fef2f2", bd: "#fecaca", c: "#991b1b" });
    if (r.created !== "Today" && r.status === "Pending") flags.push({ type: "aging", title: "Approval aging", body: "Pending for " + r.created + " without a decision. Response is overdue.", bg: "#fff7ed", bd: "#fed7aa", c: "#92400e" });
    return { r, flags };
            ...
            <div style={{ display:"flex", flex:1, overflow:"hidden", height:"100%" }}>
              <div style={{ flex:1, overflow:"auto", minWidth:0 }}>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", background:"#fff" }}>
                  <thead><tr style={{ borderBottom:"1px solid #f1f5f9", background:"#f8fafc" }}>
                    {["Request","Site / Supplier","Amount","Status",""].map(h=><th key={h} style={{ fontSize:10, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:".06em", padding:"8px 12px", textAlign:"left" }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {filteredReqs.map(r => {
                      ...
                      return (
                        <tr key={r.id} onClick={()=>setSelId(r.id)} style={{ borderBottom:"1px solid #f8fafc", cursor:"pointer", background: selId===r.id ? "#f0f9ff" : "#fff" }}>
                          ...
                          <td style={{ padding:"10px 12px" }}><div style={{ fontSize:12, fontWeight:600 }}>{r.venue}</div><div style={{ fontSize:11, color:"#64748b" }}>{r.sup} · {r.cat}</div></td>
                          ...
                          <td style={{ padding:"10px 12px" }}>{r.status==="Pending"&&<button onClick={e=>{e.stopPropagation();setSelId(r.id);setPage("approvals");}} style={{ padding:"5px 10px", borderRadius:6, border:"1px solid #e2e8f0", background:"#f8fafc", fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Review</button>}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              {sel && (
                <div style={{ width:280, borderLeft:"1px solid #e2e8f0", background:"#fff", overflow:"auto", padding:16, flexShrink:0 }}>
                  ...
                  {sel.status==="Pending" && <button onClick={()=>{setPage("approvals");}} style={{ width:"100%", marginTop:12, background:"#18b39b", color:"#fff", border:"none", borderRadius:8, padding:9, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Go to approval →</button>}
                </div>
              )}
            </div>
          )}
              ...
              <div style={{ width:270, borderRight:"1px solid #e2e8f0", overflow:"auto", background:"#fff", flexShrink:0 }}>
                ...
                {reqs.filter(r=>r.status!=="Pending").length > 0 && <>
                  ...
                  {reqs.filter(r=>r.status!=="Pending").map(r=>(
                    <div key={r.id} style={{ padding:"10px 14px", borderBottom:"1px solid #f8fafc", opacity:.5 }}>
          ...
          {page === "queue" && (
            <div style={{ display:"flex", flex:1, overflow:"hidden", height:"100%" }}>
              <div style={{ flex:1, overflow:"auto", padding:"14px 16px", background:"#f8fafc" }}>
                ...
                {approved.map(r=>(
                  <div key={r.id} onClick={()=>setSelId(r.id)} style={{ background:"#fff", border:`1px solid ${selId===r.id?"#bfdbfe":"#e2e8f0"}`, borderLeft:`3px solid ${selId===r.id?"#0c2d5b":"transparent"}`, borderRadius:10, padding:"14px 16px", marginBottom:8, cursor:"pointer", background: selId===r.id?"#f0f9ff":"#fff" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div>
                        <div style={{ fontSize:12, fontWeight:700 }}>{r.id} · {r.venue}</div>
                        ...
                        <div style={{ marginTop:7, display:"flex", gap:5, flexWrap:"wrap" }}>
                          <span style={{ display:"inline-block", padding:"2px 7px", borderRadius:5, fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:".03em", background: r.pref?"#f0fdf4":"#fef2f2", color: r.pref?"#166534":"#dc2626" }}>{r.pref?"Preferred":"Off-preferred"}</span>
                          <span style={{ display:"inline-block", padding:"2px 7px", borderRadius:5, fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:".03em", background:"#f1f5f9", color:"#64748b" }}>Approved {r.created} ago</span>
                        </div>
                      </div>
                    ...
                    {r.note && <div style={{ marginTop:10, padding:"8px 10px", background:"#fffbeb", border:"1px solid #fde68a", borderRadius:7, fontSize:11, color:"#92400e", lineHeight:1.5 }}><strong style={{ fontSize:10, textTransform:"uppercase", letterSpacing:".05em" }}>Approver note: </strong>{r.note}</div>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {page === "control" && (
            <div style={{ display:"flex", flex:1, overflow:"hidden", height:"100%" }}>
              <div style={{ flex:1, overflow:"auto", padding:"14px 16px", background:"#f8fafc" }}>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, marginBottom:14 }}>
                  {[
                    { l:"High severity", v:EXCEPTIONS.filter(e=>e.flags.some(f=>f.type==="budget")).length, bg:"#fef2f2", bd:"#fecaca", c:"#dc2626" },
                    { l:"Medium severity",v:EXCEPTIONS.filter(e=>e.flags.some(f=>f.type==="supplier"||f.type==="aging")).length, bg:"#fff7ed", bd:"#fed7aa", c:"#b45309" },
                ...
                {EXCEPTIONS.length === 0 && <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:10, padding:"32px", textAlign:"center", color:"#94a3b8" }}><div style={{ fontSize:24, marginBottom:8 }}>✓</div><div style={{ fontWeight:600, fontSize:13, color:"#475569" }}>No exceptions</div><div style={{ fontSize:12 }}>All requests are within policy</div></div>}
                {EXCEPTIONS.map(({r,flags})=>(
                  <div key={r.id} onClick={()=>setSelId(r.id)} style={{ background: selId===r.id?"#f0f9ff":"#fff", border:`1px solid ${selId===r.id?"#bfdbfe":"#e2e8f0"}`, borderLeft:`4px solid ${flags.some(f=>f.type==="budget")?"#ef4444":flags.some(f=>f.type==="supplier"||f.type==="aging")?"#f59e0b":"#a78bfa"}`, borderRadius:10, padding:"12px 14px", marginBottom:8, cursor:"pointer" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div>
                        ...
                        <div style={{ display:"flex", flexWrap:"wrap", gap:4, marginTop:7 }}>
                          {flags.map(fl=><span key={fl.type} style={{ display:"inline-block", padding:"2px 7px", borderRadius:5, fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:".03em", background:fl.bg, color:fl.c }}>{fl.title}</span>)}
                        </div>
                      </div>
                  ...
                  {compSites.map(s=>{
                    ...
                    return (
                      <div key={s.l} style={{ marginBottom:12 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, fontWeight:600, marginBottom:3 }}><span>{s.l}</span><span style={{ color, fontWeight:700 }}>{s.v}% {s.v>=85?"✓":s.v>=70?"↓":"↓↓"}</span></div>
                        ...
                        <div style={{ fontSize:10, color:"#94a3b8", marginTop:2 }}>{s.v>=85?"On target":s.v>=70?"Below target — review off-preferred orders":"Needs attention"}</div>
                      </div>
                    );
                  })}
                ...
                <div style={{ background:"#fff", border:"1px solid #e2e8f0", borderRadius:10, padding:"14px 16px" }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                    {[
                      ...
                      ["Requests processed", rd.requests],
